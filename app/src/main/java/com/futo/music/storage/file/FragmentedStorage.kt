...
class FragmentedStorage {
    companion object {
        ...
        @Transient
        val jsonSerializer = Json { this.ignoreUnknownKeys = true; };
        ...
        inline fun <reified  T> store(name: String, serializer: StoreSerializer<T>, prettyName: String? = null, parentDir: File? = null): ManagedStore<T> {
            return ManagedStore(name, parentDir ?: _filesDir!!, typeOf<T>(), serializer);
        }
        ...
        inline fun <reified T> replace(text: String, verify: Boolean = true): T where T: FragmentedStorageFile {
            if(verify) {
                ...
                if (parsed == null) {
                    throw kotlin.IllegalStateException("Failed to import type [${T::class.java.name}]");
                }
            }
            ...
            synchronized(_cachedFiles) {
                ...
                val file = cachedFile?.getUnderlyingFile() ?: File(
                    _filesDir,
                    "${T::class.java.name}.json"
                );
                file.writeText(text);
        ...
        inline fun <reified T> load(): T where T : FragmentedStorageFile {
            if (_filesDir == null) {
                synchronized(_initLock) {
                    if(_filesDir == null)
                        FragmentedStorage.initialize(RootApplication.applicationContext.filesDir);
                }
                if(_filesDir == null)
                    throw kotlin.Exception("Files dir should be initialized before loading a file.")
            }
